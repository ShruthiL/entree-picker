...
Restaurant.limit(2).each_with_index do |res, index|
  Review.create(rating: 4.6, comments: "This is the best dish I ever had", user: User.all[index], menu_item: MenuItem.all[9])
end

Restaurant.limit(2).each_with_index do |res, index|
  Review.create(rating: 4.8, comments: "My boyfriend just loved this dish", user: User.all[index], menu_item: MenuItem.all[10])
end

Restaurant.limit(4).each_with_index do |res, index|
  Review.create(rating: 4.8, comments: "I want to try this dish again, just love it", user: User.all[index], menu_item: MenuItem.all[7])
end
